const welcomes = [
  "Hello", "Witaj", "Bonjour", "Hola", "Ciao",
  "Hallo", "こんにちは", "안녕하세요", "Привет",
  "مرحبا", "שלום", "नमस्ते", "Olá",
];
...
function goToStep(n) {
  const current = document.querySelector(`.step-panel[data-panel="${currentStep}"]`);
  if (current) {
...
btnKeyboardNext.addEventListener("click", async () => {
  ...
  const timeFormat     = document.querySelector('input[name="timeFormat"]:checked')?.value || "24h";
...
btnNetworkNext.addEventListener("click", async () => {
  const type        = document.querySelector('input[name="networkType"]:checked')?.value || "none";
...
btnFinish.addEventListener("click", async () => {
  ...
  if (password !== password2) return showError("finish-error", "Hasła nie są zgodne.");
  if (password.length < 6)    return showError("finish-error", "Hasło musi mieć co najmniej 6 znaków.");
